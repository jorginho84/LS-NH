...
foreach x of numlist 1 4 7{
	...
	replace l_t`x'=(168-40) if d_CC2_t`x'==1 & age_t`x'<=5
	replace l_t`x'=(133-hours_t`x') if d_CC2_t`x'==0 & age_t`x'>5
	replace l_t`x'=(133-40) if d_CC2_t`x'==1 & age_t`x'>5
}
...
mat inputs_moments_young=J(4,1,.)
mat inputs_moments_old=J(4,1,.)
mat inputs_moments=J(1,1,.)

corr skills_t2  lhwage_t0 if p_assign=="C"
mat inputs_moments[1,1] = r(rho)

corr d_skills_t2 d_skills_t5 if p_assign == "C" & age_t1>5
mat inputs_moments_old[1,1] = r(rho)

corr d_skills_t2 d_skills_t5 if p_assign == "C" & age_t1<=5
mat inputs_moments_young[1,1] = r(rho)
...
*/ id_child age_t1 age_t4 age_t7 d_skills_t1 d_skills_t4 d_skills_t7 p_assign age_year2

reshape long incomepc_t skills_t d_skills_t l_t d_CC2_t age_t, i(id_child) j(year)

reg d_skills_t incomepc_t l_t if p_assign=="C" & year<7 & age_t<=5
mat inputs_moments_young[2,1] = _b[incomepc_t]
mat inputs_moments_young[3,1] = _b[l_t]

reg d_skills_t incomepc_t l_t if p_assign=="C" & year<7 & age_t>5
mat inputs_moments_old[2,1] = _b[incomepc_t]
mat inputs_moments_old[3,1] = _b[l_t]

reg d_skills_t d_CC2_t if  p_assign=="C" & year<7 & age_t<=5
mat inputs_moments_young[4,1] = _b[d_CC2_t]

reg d_skills_t d_CC2_t if  p_assign=="C" & year<7 & age_t>5
mat inputs_moments_old[4,1] = _b[d_CC2_t]
...
matrix betas_theta =prob_inc_t2\prob_inc_t5\inputs_moments_young\inputs_moments_old\inputs_moments
