...
d30_t2 = choices['ssrs_t2_matrix']>=4
d30_t5 = choices['ssrs_t2_matrix']>=4
ate_d30_t2 = np.mean(np.mean(d30_t2[passign[:,0]==1,:],axis=0) - np.mean(d30_t2[passign[:,0]==0,:],axis=0))
ate_d30_t5 = np.mean(np.mean(d30_t5[passign[:,0]==1,:],axis=0) - np.mean(d30_t5[passign[:,0]==0,:],axis=0))
...
plt.show()
fig.savefig('/home/jrodriguez/understanding_NH/results/Model/fit/ate_theta.pdf', format='pdf')
plt.close()
...
plt.show()
fig.savefig('/home/jrodriguez/understanding_NH/results/Model/fit/ate_theta_young.pdf', format='pdf')
plt.close()
